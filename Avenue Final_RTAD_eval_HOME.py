...
def createNewTrainFS (yoloFrameData, psnrFrameError):
    ...
    if len(yoloFrameData) != 0: # if yolo detect >= 1 object
        for obj in yoloFrameData:
            ...
            if float(obj[1]) >= 30: # Filter Class that it's confidence score < 0.6
                # print("my Object: ", obj)
...
fileAnnotation = open("/content/input/UIT-ConStorexAbnormal/Annotation.txt", "r")
...
for i in range(39, 113+1, 2):
    my_list.append(str(i).zfill(3))
...
for line in Lines:
    ...
    splittedDir = line[0].split("_")
    vidLink = ""
    postfix = line[0].split("/")[1]

    if (line[-3] == "Normal"):
        vidLink = "Normal/{}/{}/{}".format(splittedDir[1], splittedDir[2], postfix)
    else:
        vidLink = "Shoplifting/{}/{}".format(splittedDir[1], postfix)

    totalVid[splittedDir[-2]] = vidLink

    # totalFrames = line[1]
    # startFrame = line[3]
    # stopFrame = line[4]
    # char = line[2]
    # print("Total Frames {} - Start {} - Stop {} ".format(totalFrames, startFrame, stopFrame))
    # print("Dir {} - Char {} ".format(dir, char))
...
# Training
for item in my_list:
    dir = "/content/input/UIT-ConStorexAbnormal/" + totalVid[item]
    print("dir ", dir)
    del totalVid[item]
    ...
    while(cap.isOpened()):
        ...
        if count % 50 == 0:
            fps = int(1/(time.time() - prev_time))
            # print("FPS: {}".format(fps))
            print("Video {} - FPS {} - count {}".format(item, fps, count))
...
# Testing
for item in totalVid:
    dir = "/content/input/UIT-ConStorexAbnormal/" + totalVid[item]
    print("dir ", dir)

    # dir = r"/content/input/ConStorexAbnormal/Normal_Extended/{}".format(item)
    ...
    resultVid = cv2.VideoWriter('/content/output/vid/{}.avi'.format(item), 
    ...
    resultVid.release()
    print("\n")
    print("Video {}".format(item))
    print(predict)
    f.write("Video {}".format(item))
    f.write(str(predict) + "\n")

    file.write("Video {}".format(item))

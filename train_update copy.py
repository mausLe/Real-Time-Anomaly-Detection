...
with tf.variable_scope('generator', reuse=None):
    ...
    train_psnr_error, shape = psnr_error(gen_frames=train_outputs, gt_frames=train_gt)
...
# train_psnr_error = tf.cast(train_psnr_error, dtype=tf.float32)
...
with tf.Session(config=config) as sess:
    ...
    while _step < iterations:
        try:
            ...
            _, _g_lr, _step, _lp_loss, _gdl_loss, _adv_loss, _flow_loss, _g_loss, _train_psnr = sess.run(
                [g_train_op, g_lrate, g_step, lp_loss, gdl_loss, adv_loss, flow_loss, g_loss, train_psnr_error])

            """
            print("-"*40)
            print("g_step value: ", sess.run(g_step))
            print("Finish: 1")
            print("g_lrate value: ", sess.run(g_lrate))
            print("Finish: 2")
            print("g_train_op value: ", sess.run(g_train_op))
            print("Finish: 3")
            print("lp_loss value: ", sess.run(lp_loss))
            print("Finish: 4")
            print("gdl_loss value: ", sess.run(gdl_loss))
            print("Finish: 5")
            print("adv_loss value: ", sess.run(adv_loss))
            print("Finish: 6")
            print("flow_loss value: ", sess.run(flow_loss))
            print("Finish: 7")
            print("g_loss value: ", sess.run(g_loss))
            print("Finish: 8")
            print("train_psnr_error value: ", sess.run(train_psnr_error))
            print("Finish: 9")
            # print("summary_op value: ", sess.run(summary_op))
            print("Finish: 10")

...
def createNewTrainFS (yoloFrameData, psnrFrameError, ind):
    """
    yoloFrameData :
    psnrFrameError : 
    """
    global Train_FS, trainingClasses, myObj, totalFrame
    myFrame = []
    ...
    myObj.append(myObject)
    totalFrame.append(myFrame)
...
for imageDir in images: # Apple Store stolen images[600:1000]
    ...
    frame_FS = createNewTrainFS(detections, psnr, ind)
    ind += 1

f = open("/content/myObject.txt", "a")
f.write(str(myObj))
f.close()
...
"""
totalFrame_N = totalFrame*1
for j in range(len(totalFrame_N)):
    for i in range(0,4):
        totalFrame_N[j][:,i] = (totalFrame[j][:,i] - g_min[i])/(g_max[i]-g_min[i])

    for i in range(1,4):                            
        totalFrame_N[j][:,i] = 0.2*(totalFrame_N[j][:,i])

    for i in range(4,5):                            
        totalFrame_N[j][:,i] = 0*(totalFrame_N[j][:,i])
"""
